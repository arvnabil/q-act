...
function getLogoConfigForQuotation(q, isPdfMode = false) {
  const email = q?.creator?.email || q?.sales?.email || q?.created_by_email || '';
...
const translateTerm = (term, lang) => {
  ...
  let t = term.trim();
  if (/^\d+\.\s*Syarat\s*&\s*Ketentuan$/i.test(t) || /^Syarat\s*&\s*Ketentuan$/i.test(t)) return null;
  t = t.replace(/^\d+\.\s*/, '');
  const rules = [
...
const TRANSLATIONS = {
  id: {
    ...
    no: 'NO', sku: 'SKU', brandName: 'NAMA BRAND', descProduct: 'Deskripsi Produk',
    productDisplay: 'Tampilan Produk', qty: 'JML', price: 'HARGA', totalPrice: 'TOTAL HARGA',
    emptyItem: 'Belum ada item produk.',
    totalAllProduct: 'TOTAL', ppn: 'PPN (VAT) 11%', grandTotal: 'GRAND TOTAL',
    termsTitle: 'Syarat & Ketentuan', bankAcc: 'Nomor Rekening :',
    branchOffice: 'Kantor Cabang Utama Alam Sutera ( Tangerang Selatan )',
    approvalBy: 'Disetujui oleh :', signatureStamp: 'Tanda Tangan & Cap Pelanggan', noImage: 'Tidak Ada Gambar',
  },
  en: {
    quotationNo: 'QUOTATION NO',
    to: 'To :', sender: 'Sender :', priceInIdr: 'Price in IDR',
    no: 'NO', sku: 'SKU', brandName: 'BRAND NAME', descProduct: 'Product Description',
    productDisplay: 'Product Display', qty: 'QTY', price: 'PRICE', totalPrice: 'TOTAL PRICE',
    emptyItem: 'No product items available.',
    totalAllProduct: 'TOTAL ALL PRODUCTS', ppn: 'VAT 11%', grandTotal: 'GRAND TOTAL',
    termsTitle: 'Terms & Conditions', bankAcc: 'Bank Account :',
    branchOffice: 'Alam Sutera Main Branch ( South Tangerang )',
    approvalBy: 'Approved by :', signatureStamp: 'Customer Signature & Stamp', noImage: 'No Image',
  }
};

export function generateQuotationHTML(q, withImage = false, bankAccount = null, lang = 'id', includePrintScript = true, isPdfMode = false, logoUrl = null) {
  ...
  items.forEach((item, idx) => {
    ...
    const brandName = item.brand || prod?.brand?.name || prod?.brands?.name || 'ACTIV';
    let imageUrl = item.image_url || item.image || prod?.image_url || prod?.image || prod?.image_path || null;
    ...
    const rawDesc = item.description || prod?.description || '';
    const desc = translateDynamicText(rawDesc, lang);
    const price = (item.price || 0) * taxMultiplier;
    const qty = item.qty || 1;
    const itemTotal = qty * price;
    const imageHtml = imageUrl
      ? `<div style="max-width: 75px; max-height: 42px; overflow: hidden; margin: 0 auto; display: flex; align-items: center; justify-content: center;"><img src="${imageUrl}" style="max-width: 100%; max-height: 100%; object-fit: contain;" alt="${productName}" /></div>`
    ...
    itemRows += `
      <tr>
        ...
        ${withImage ? `<td style="padding: 2px; text-align: center; vertical-align: middle;">${imageHtml}</td>` : ''}
        ...
        <td style="font-size: ${tdFontSize}; vertical-align: top; padding: ${tdPadding};">
          <table style="width: 100%; border: none !important; border-collapse: collapse; margin: 0; padding: 0; background: transparent !important;">
            <tr><td style="border: none !important; padding: 0; text-align: left; width: 20px;">Rp</td><td style="border: none !important; padding: 0; text-align: right;">${formatCurrencyDecimals(price)}</td></tr>
          </table>
        </td>
        <td style="font-size: ${tdFontSize}; vertical-align: top; padding: ${tdPadding};">
          <table style="width: 100%; border: none !important; border-collapse: collapse; margin: 0; padding: 0; background: transparent !important;">
            <tr><td style="border: none !important; padding: 0; text-align: left; width: 20px;">Rp</td><td style="border: none !important; padding: 0; text-align: right;">${formatCurrencyDecimals(itemTotal)}</td></tr>
          </table>
        </td>
      </tr>
    `;
  ...
  const rawTerms = q.terms && Array.isArray(q.terms) && q.terms.length > 0 ? q.terms : [];
  const termsList = rawTerms.map(term => translateTerm(term, lang)).filter(Boolean);
      ...
      <head>
        ...
        <style>
          *, html, body, div, table, tr, th, td {
          ...
          @page { margin: ${isPdfMode ? '0' : '25mm 20mm'}; size: A4 portrait; }
          body {
            font-family: Arial, Helvetica, sans-serif; color: #111; margin: 0;
            padding: ${isPdfMode ? '25px 35px' : '0'};
            font-size: ${baseFontSize}; line-height: 1.2;
          ...
          .table-items { width: 100%; border-collapse: collapse; margin-bottom: 0px; table-layout: fixed; }
          .table-items th, .table-items td { border: 1px solid ${tableBorderColor} !important; padding: ${tdPadding}; box-sizing: border-box; }
          .table-items th { background-color: #f1f5f9 !important; color: #000000 !important; padding: ${thPadding}; font-size: ${tdFontSize}; font-weight: bold; text-transform: uppercase; text-align: center; }
            ...
            <tr>
              <td class="header-left">
                ...
                <div style="font-size: 8.5px; line-height: 1.35;">
                  ...
                  <div style="font-weight: bold;">${creatorObj?.name || 'Sales'}</div>
                  <div style="font-weight: bold; text-transform: uppercase;">${COMPANY.name} (${COMPANY.brand})</div>
                  <div>Mobile : ${creatorObj?.mobile || '-'}</div>
              ...
              <td class="header-right">
                <div style="width: ${headerRightWidth}; margin-left: auto; text-align: left;">
                  <img src="${resolvedLogoUrl}" alt="${COMPANY.brand}" style="max-height: ${logoMaxHeight}; max-width: ${logoMaxWidth}; display: block; margin-bottom: 4px; object-fit: contain;" />
            ...
            <table class="table-items">
              ...
              <tbody>
                ...
                <tr style="font-weight: bold;">
                  <td colspan="${withImage ? 5 : 4}" style="border: none !important; background: transparent !important;"></td>
                  ...
                  <td style="font-size: ${tdFontSize}; vertical-align: middle; border-top: 1px solid ${tableBorderColor} !important; padding: ${tdPadding};">
                    <table style="width: 100%; border: none !important; border-collapse: collapse; margin: 0; padding: 0; background: transparent !important;">
                      <tr><td style="border: none !important; padding: 0; text-align: left; width: 20px;">Rp</td><td style="border: none !important; padding: 0; text-align: right;">${formatCurrencyDecimals(subtotal)}</td></tr>
                    </table>
                  </td>
                </tr>
                ${(calcTax && showTax) ? `
                  <tr style="font-weight: bold;">
                    ...
                    <td style="font-size: ${tdFontSize}; vertical-align: middle; padding: ${tdPadding};">
                      <table style="width: 100%; border: none !important; border-collapse: collapse; margin: 0; padding: 0; background: transparent !important;">
                        <tr><td style="border: none !important; padding: 0; text-align: left; width: 20px;">Rp</td><td style="border: none !important; padding: 0; text-align: right;">${formatCurrencyDecimals(ppn)}</td></tr>
                      </table>
                    </td>
                  </tr>
                ` : ''}
                ${(calcPph && showPph) ? `
                  <tr style="font-weight: bold;">
                    ...
                    <td style="font-size: ${tdFontSize}; vertical-align: middle; padding: ${tdPadding};">
                      <table style="width: 100%; border: none !important; border-collapse: collapse; margin: 0; padding: 0; background: transparent !important;">
                        <tr><td style="border: none !important; padding: 0; text-align: left; width: 20px;">Rp</td><td style="border: none !important; padding: 0; text-align: right;">${formatCurrencyDecimals(pphAmount)}</td></tr>
                      </table>
                    </td>
                  </tr>
                ` : ''}
                <tr style="font-weight: bold;">
                  ...
                  <td style="font-size: ${tdFontSize}; vertical-align: middle; padding: ${tdPadding};">
                    <table style="width: 100%; border: none !important; border-collapse: collapse; margin: 0; padding: 0; background: transparent !important;">
                      <tr><td style="border: none !important; padding: 0; text-align: left; width: 20px;">Rp</td><td style="border: none !important; padding: 0; text-align: right;">${formatCurrencyDecimals(grand)}</td></tr>
                    </table>
                  </td>
                </tr>
              </tbody>
...
export function printQuotation(q, withImage = false, bankAccount = null, lang = 'id') {
...
export function downloadQuotationPDF(q, withImage = false, bankAccount = null, lang = 'id') {
  const qId = q.id || 'document';
  const custName = q.customer?.name || '';
  const rawFilename = custName ? `Quotation ${qId} - ${custName}` : `Quotation ${qId}`;
